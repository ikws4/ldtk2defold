...
function M.dirtyLine(line, data)
    ...
    elseif separator == ":" then
        ...
        elseif string.match(value, "^[A-Z_]*$") then
            ...
            if key == "collision_group" then
                value = reQuote(value)
            end
        elseif not ENUM_KEYS[key] then
            value = reQuote(value)
        end
        separator = ": "
    elseif separator == "}" then
...
local function makeCleanLinesForTable(t, lines, indent, parentKey, tilemap)
    local i = 0
    ...
    for k, v in pairs(t) do
        if not IGNORE_KEYS[k] then
            ...
            if type(v) == "table" then
                if v[1] then
                    if not tilemap then doUnIndent = false end
                    makeCleanLinesForTable(v, lines, indent, k, tilemap)
                else
                    doUnIndent = not tilemap
                    table.insert(lines, reIndent(string.format("%s {", k), indent))
                    makeCleanLinesForTable(v, lines, indent + 1, k, tilemap)
                end
            elseif isVecOrQuat(v) then
                table.insert(lines, reIndent(string.format("%s {", k), indent))
                local t = { x = v.x, y = v.y, z = v.z }
                if isQuat(v) then t.w = v.w end
                makeCleanLinesForTable(t, lines, indent + 1, k, tilemap)
            else -- v == a normal value.
                if k == parentKey then doUnIndent = false end
                if tonumber(v) and not INTEGER_KEYS[k] then
                    v = formatNumber(v)
                end
...
function M.tableToCleanLines(data, tilemap)
    local lines = {}
    makeCleanLinesForTable(data, lines, 0, nil, tilemap)
    return lines
end
...
local function createTilemapCleanLines(data, lines, indent, parentKey)
    local i = 0
    local doUnIndent = true
    for k, v in pairs(data) do
        if not IGNORE_KEYS[k] then
            ...
            if type(v) == "table" then
                if v[1] then
                    -- doUnIndent = false
                    createTilemapCleanLines(v, lines, indent, k)
                else
                    doUnIndent = false -- lines[1] ~= nil
                    table.insert(lines, reIndent(string.format("%s {", k), indent))
                    createTilemapCleanLines(v, lines, indent + 1, k)
                end
            else -- v == a normal value.
                if k == parentKey then doUnIndent = false end
                if tonumber(v) and not INTEGER_KEYS[k] then
...
function M.encodeTilemap(file, data)
    local lines = M.tableToCleanLines(data, true)
    local dirtyingData = { dataLevel = 0, indent = 0, lines = {}, dataIndentLevels = {} }
    for i, line in ipairs(lines) do
        M.dirtyLine(line, dirtyingData)
    end
